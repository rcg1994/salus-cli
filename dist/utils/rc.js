...
const exits = (0, _util.promisify)(_fs2.default.access);
...
const get = exports.get = async key => {
  const exit = await exits(_const.RC);
  let opts;
  if (exit) {
    opts = await readFile(_const.RC, 'utf8');
...
const set = exports.set = async (key, value) => {
  ...
  if (exit) {
    ...
    opts = Object.assign(_const.DEFAULTS, { [key]: value });
  }
  await writeFile(_const.RC, (0, _ini.encode)(opts), 'utf8');
};
